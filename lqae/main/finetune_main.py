...
def main(argv):
    ...
    def combine_representation(output):
        ...
        # Stop gradient on encoder_embedding in Finetuning
    ...
    cross_entropy_loss_fn = partial(cross_entropy_loss, smoothing_factor = 0. if FLAGS.mixup_alpha > 0. else FLAGS.label_smoothing)
    ...
    def train_step_fn(state, rng, image, label):
        ...
        def loss_fn(params):
            ...
            loss = cross_entropy_loss_fn(logits, augmented_label)
    ...
    if FLAGS.load_checkpoint != "":
        ...
        def get_weight_decay_mask(params):
            ...
            def decay(key):
                return all([k not in backbone.no_decay_list() for k in key])
